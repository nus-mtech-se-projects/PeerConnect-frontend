...
import { authenticate, setupAndGoto, mockRoute, createGroupData, GROUP_ID, OWNER_USER_ID, MEMBER_USER_ID } from './helpers/group-fixtures';
...
test.describe('Email notifications on group actions', () => {
  ...
  test('invite member sends email notification', async ({ page }) => {
    ...
    await page.getByPlaceholder('student@u.nus.edu').fill('newmember@u.nus.edu');
